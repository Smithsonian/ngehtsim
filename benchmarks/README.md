...
Raster scenarios use the native FINUFFT transform through the default
``auto`` setting. Compare it with the exact direct transform by running the
same image scenario twice:

```bash
MPLBACKEND=Agg python3 benchmarks/benchmark_synthetic_data.py \
  --scenario eht2017_image_clean --transform-backend finufft \
  --output benchmarks/results/eht2017_image_clean_finufft.json

MPLBACKEND=Agg python3 benchmarks/benchmark_synthetic_data.py \
  --scenario eht2017_image_clean --transform-backend direct \
  --output benchmarks/results/eht2017_image_clean_direct.json
```

Use ``direct`` when an exact reference transform is required. FINUFFT supports
odd-sized images, and the removed v1 ``fast`` and ``nfft`` backend names are
not accepted by ngehtsim.
