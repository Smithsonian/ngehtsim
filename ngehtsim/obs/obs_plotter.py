...
def plot_snapshot(obs, obsgen, metric, filename='metric.png', timetype='UTC', fov=100.0,
                  fillpix=10, logmid=1.5, logwid=0.525, stokes='I', artype='mean',
                  weighting='natural', robust=0.0, ylim=None):
    """
    ...
    if (metric.lower() == 'ff'):
        ...
        t, y = cm.calc_ff_continuous(obs, longest_BL, start_time=start_time, end_time=end_time,
                                     snapshot_interval=snapshot_interval, fov=fov, fillpix=fillpix)
    if (metric.lower() == 'bff'):
        longest_BL = const.D_Earth/(const.c/obs.rf)
        t, y = cm.calc_bff_continuous(obs, longest_BL, start_time=start_time, end_time=end_time,
                                      snapshot_interval=snapshot_interval, fov=fov, fillpix=fillpix,
                                      logmid=logmid, logwid=logwid, stokes=stokes)
    if (metric.lower() == 'lcg'):
        dummy_circ_res = ((const.c/obs.rf) / const.D_Earth) / eh.RADPERUAS
        t, y = cm.calc_lcg_continuous(obs, start_time=start_time, end_time=end_time,
                                      snapshot_interval=snapshot_interval, dummy_circ_res=dummy_circ_res)
    if (metric.lower() == 'pss'):
        t, y = cm.calc_pss_continuous(obs, start_time=start_time, end_time=end_time,
                                      snapshot_interval=snapshot_interval)
    if (metric.lower() == 'ar'):
        t, y = cm.calc_ar_continuous(obs, start_time=start_time, end_time=end_time,
                                     snapshot_interval=snapshot_interval, artype=artype,
