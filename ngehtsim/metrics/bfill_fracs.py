...
@authors: Daniel Palumbo, Dom Pesce
"""
...
def grid_obs(obs, N, psize, stokes='I'):
    ...
    rangehere = [[-int(1.2*N)/2, int(1.2*N)/2], [-int(1.2*N)/2, int(1.2*N)/2]]
    outtemp, dum1, dum2, dum3 = binned_statistic_2d(x, y, z, statistic='sum', bins=int(1.2*N), range=rangehere)
    outnorm, dum1, dum2, dum3 = binned_statistic_2d(x, y, znorm, statistic='sum', bins=int(1.2*N), range=rangehere)
...
def obs_fill(obs, longest, fov=100, N=10, logmid=1.5, logwid=0.525, stokes='I'):
    ...
    outconv = fftconvolve(grid_obs(obs, Npix, psize, stokes=stokes),
                          unit_disk(conv_radius, int(round(2*conv_radius+1))), mode='full')
